...
class VideoConverter(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def format_to_codec(value):
            for key, value in self.video_codec.items():
                if key == self.list_format.get():
                    ...
                    for option in new_menu:
                        menu.add_command(
                            label=option, command=tk._setit(self.list_video, option)
                        )
            ...
            for key, value in self.audio_codec.items():
                if key == self.list_format.get():
                    ...
                    for option in new_menu:
                        menu.add_command(
                            label=option, command=tk._setit(self.list_audio, option)
                        )
            ...
            try:
                ...
                def convert_button():
                    try:
                        ...
                        self.clip = VideoFileClip(rf"{self.dropped_file}")

                        # Конвертируем в MP4
                        self.clip.write_videofile(
                            f"{self.default_name}.{self.list_format.get().lower()}",
                            codec=f"{self.list_video.get()}",
                            audio_codec=f"{self.list_audio.get()}",
                        )
                        log_error(
                            self,
        ...
        self.label_dropdown = tk.Label(
            self, text="Выберите формат и кодек файла:", font=("Helvetica", 14)
        )  # виджет с текстом
        ...
        # рекомендация
        self.label_recommendation = tk.Label(
            self,
            text="Если не знаете какой кодек ставить\nто рекомендуется ставить первый",
            font=("Helvetica", 12),
        )
        ...
        self.dropdown1 = tk.OptionMenu(
            self, self.list_format, *self.formats, command=format_to_codec
        )
